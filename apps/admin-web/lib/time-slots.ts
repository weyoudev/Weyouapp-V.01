/**
 * 30-minute time slot options for pickup (customer simulator and any order forms).
 * Display: "9:00 AM"; API value: "09:00-09:30".
 */
...
export function getTimeSlotOptions(): TimeSlotOption[] {
  ...
  for (let hour = SLOT_START_HOUR; hour < SLOT_END_HOUR; hour++) {
    for (let minute = 0; minute < 60; minute += SLOT_INTERVAL_MINUTES) {
      ...
      options.push({ label: formatTimeLabel(hour, minute), value: `${start}-${end}` });
    }
  }
  return options;
}
